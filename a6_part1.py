...
def train_model(X_train, y_train):
    ...
    print(f"\nEquation: Scores = {model.coef_[0]:.2f} x Hours + {model.intercept_:.2f}")
...
def visualize_results(X_train, y_train, X_test, y_test, predictions, model):
    ...
    plt.scatter(X_train, y_test,color='green',alpha=0.7, label = 'Test Data (Actual)')
    ...
    plt.savefig('student_scores_predictions.png', dpi=300 , bbox_inches='tight')
    # TODO: Show the plot
    plt.show()
...
def make_prediction(model, hours):
    ...
    print(f"\n=== New Prediction ===")
    print(f"If hours spent studied are {hours}, predicted scores: {predicted_scores:.2f}")
